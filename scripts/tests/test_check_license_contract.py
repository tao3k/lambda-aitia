# SPDX-FileCopyrightText: 2026 tao3k team and Contributors
#
# SPDX-License-Identifier: Apache-2.0 AND LGPL-2.1-or-later
"""Tests for the Lambda Aitia publication license contract."""

from __future__ import annotations

import importlib.util
import subprocess
import sys
import unittest
from pathlib import Path


ROOT = Path(__file__).resolve().parents[2]
SCRIPT = ROOT / "scripts" / "check_license_contract.py"
EXPRESSION = "Apache-2.0 AND LGPL-2.1-or-later"


def license_contract_module():
    spec = importlib.util.spec_from_file_location("check_license_contract", SCRIPT)
    assert spec is not None
    assert spec.loader is not None
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class LicenseContractTest(unittest.TestCase):
    def test_exact_dual_license_expression_is_authoritative(self) -> None:
        contract = license_contract_module()

        self.assertEqual(contract.EXPRESSION, EXPRESSION)
        self.assertIn(
            f"SPDX-License-Identifier: {EXPRESSION}",
            (ROOT / "LICENSE").read_text(encoding="utf-8").splitlines()[:3],
        )
        self.assertTrue((ROOT / "LICENSES" / "Apache-2.0.txt").is_file())
        self.assertTrue((ROOT / "LICENSES" / "LGPL-2.1-or-later.txt").is_file())

    def test_repository_license_contract_is_closed(self) -> None:
        completed = subprocess.run(
            [sys.executable, str(SCRIPT)],
            cwd=ROOT,
            check=False,
            capture_output=True,
            text=True,
        )

        self.assertEqual(completed.returncode, 0, completed.stderr)
        self.assertIn(f"license-contract: ok", completed.stdout)
        self.assertIn(EXPRESSION, completed.stdout)


if __name__ == "__main__":
    unittest.main()
